<script lang="ts">
  ...
  import { request, message, publicURL, localDateTime, APIError, type EventRecord } from '../lib/api';
  ...
  async function remove() {
    ...
    catch (cause) { error = message(cause); busy = false; }
  }
</script>
...
<main class="workspace editor">
  ...
  {:else}
    ...
    {#if event}<section class="delete-panel" aria-labelledby="delete-heading"><h2 id="delete-heading">Delete event</h2>
      {#if confirming}<p>Delete “{event.name}” permanently? Its public link will stop working. This cannot be undone.</p><div class="actions"><button class="danger" disabled={busy} bind:this={confirmButton} onclick={remove}>Delete event permanently</button><button class="secondary" disabled={busy} onclick={() => confirming = false}>Keep event</button></div>
      {:else}<p>Remove this event and its public page.</p><button class="danger secondary" disabled={busy} onclick={confirmDelete}>Delete event…</button>{/if}
    </section>{/if}
  {/if}
</main>
