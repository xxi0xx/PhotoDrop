...
func TestInitializationAndRestart(t *testing.T) {
	...
	if count := migrationCount(t, db); count != 7 || before != after {
...
func TestConcurrentInitialization(t *testing.T) {
	...
	if count := migrationCount(t, openTestDB(t, dir)); count != 7 {
...
func TestGate1Upgrade(t *testing.T) {
	...
	if migrationCount(t, db) != 7 || checksum != afterChecksum || applied != afterApplied {
...
func TestGate2UpgradePreservesEventsAndSessions(t *testing.T) {
	...
	if after != before || name != "Existing event" || deleting || sessions != 1 || migrationCount(t, db) != 7 {
