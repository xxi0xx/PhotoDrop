...
let cookie = '', csrf = '';
function compose(args, active = 'local-default', configured = 's3-a,s3-b', run = execFileSync) {
  return run('docker', [...composeArgs, ...args], { stdio: 'inherit', env: {
...
function restart(active, configured) { return compose(['up', '-d', '--no-deps', '--force-recreate', '--wait', '--wait-timeout', '120', 'photodrop'], active, configured, runAsync); }
...
try {
  ...
  await restart('local-default'); // Leave the isolated app available for browser checks.
}
