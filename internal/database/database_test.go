...
func TestInitializationAndRestart(t *testing.T) {
	...
	var tables int
	if err := db.QueryRow("SELECT count(*) FROM sqlite_master WHERE type = 'table'").Scan(&tables); err != nil || tables != 12 {
		t.Fatalf("expected twelve migration/domain tables, got %d: %v", tables, err)
	}
	var before string
	...
	if count := migrationCount(t, db); count != 9 || before != after {
...
func TestConcurrentInitialization(t *testing.T) {
	...
	if count := migrationCount(t, openTestDB(t, dir)); count != 9 {
...
func TestGate1Upgrade(t *testing.T) {
	...
	if migrationCount(t, db) != 9 || checksum != afterChecksum || applied != afterApplied {
...
func TestGate2UpgradePreservesEventsAndSessions(t *testing.T) {
	...
	if after != before || name != "Existing event" || deleting || sessions != 1 || migrationCount(t, db) != 9 {
		t.Fatal("Gate 2 data/history changed during upgrade")
	}
	db.Close()
	db = openTestDB(t, dir)
	if migrationCount(t, db) != 9 {
