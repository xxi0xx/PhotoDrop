<script lang="ts">
  ...
  function transfer(item: Item, plan?: UploadPlan): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      xhr.onload = () => {
        if (plan) { finish(xhr.status >= 200 && xhr.status < 300 ? undefined : new Error('Upload did not finish. Please retry this file.')); return; }
      ...
      xhr.onabort = () => finish(new Error('Upload canceled. You can retry this file.'));
      active.add(xhr);
      // Send the original File directly, including direct-to-S3 PUTs.
      try { xhr.send(item.file); } catch { finish(new Error('Could not send this file. Please try again.')); }
    });
  }
  async function start(retry = false) {
    ...
    try {
      ...
      for (const item of selected) if (item.status === 'waiting') { item.status = 'failed'; item.error = cooling ? 'Please wait a moment and retry this file.' : 'Upload canceled. You can retry this file.'; }
      busy = false;
...
<section class="upload-panel" aria-labelledby="upload-heading">
  {#if allDone}
    <div class="upload-thanks" role="status"><span class="success-mark" aria-hidden="true">✓</span><h2 id="upload-heading" tabindex="-1" bind:this={completion}>Thanks for sharing!</h2><p>{completed} {completed === 1 ? 'file uploaded' : 'files uploaded'} successfully.</p><p class="hint">Your files are with the event host.</p></div>
    <button type="button" onclick={addMore}>Add more files</button>
  {:else}
    <h2 id="upload-heading">Share your files</h2><p class="section-intro">A moment you captured. A memory to keep.</p>
    {#if !attempted || adding}
    ...
    {#if items.length}
      {#if busy}<div class="progress-summary"><h3>Uploading files</h3><p role="status">{completed} of {items.length} complete</p><label for="overall-progress">Overall progress · {percent}%</label><progress id="overall-progress" max="100" value={percent}></progress><p class="hint">Keep this page open until your files finish.</p></div>
      {:else if attempted && failed}<div class="batch-result" role="status"><h3>{completed ? `${completed} ${completed === 1 ? 'file uploaded' : 'files uploaded'}` : "We couldn't upload these files"}</h3><p>{failed} {failed === 1 ? 'file needs' : 'files need'} another try. {#if completed}Your uploaded files are safe and will not be sent again.{/if}</p></div>{/if}
      <p class="selection-summary">{items.length} {items.length === 1 ? 'file selected' : 'files selected'} <span>· {formatBytes(totalBytes)}</span></p>
      <ul class="upload-list" aria-label="Selected files">
        {#each items as item, i}<li aria-describedby={item.error ? `upload-error-${i}` : undefined}>
          ...
          {#if item.validationError && !busy}<button type="button" class="text-button" aria-label={`Remove unsupported media ${item.file.name}`} onclick={() => items = items.filter(one => one !== item)}>Remove from selection</button>{/if}
        </li>{/each}
      </ul>
      ...
      <div class="actions upload-actions">
        ...
        {:else}
          {#if waiting}<button type="button" disabled={cooling} onclick={() => start()}>Upload {waiting} {waiting === 1 ? 'file' : 'files'}</button>{/if}
          {#if retryable}<button type="button" disabled={cooling} onclick={() => start(true)}>Retry {retryable} failed {retryable === 1 ? 'file' : 'files'}</button>{/if}
          {#if attempted && !adding}<button type="button" class="secondary" onclick={addMore}>Add more files</button>{/if}
        {/if}
      </div>
    {/if}
  {/if}
