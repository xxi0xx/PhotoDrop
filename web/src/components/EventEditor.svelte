...
<main class="workspace editor">
  ...
  {:else}
    ...
    {#if event}<section class="delete-panel" aria-labelledby="delete-heading"><h2 id="delete-heading">Danger zone</h2>
      {#if confirming}<p>Delete “{event.name}” and all its uploaded files permanently? Its public link will stop working. This cannot be undone.</p><div class="actions"><button class="danger" disabled={busy} bind:this={confirmButton} onclick={remove}>Delete event permanently</button><button class="secondary" disabled={busy} onclick={keepEvent}>Keep event</button></div>
      {:else}<p>Remove this event, its public page, and all its uploaded files.</p><button class="danger secondary" disabled={busy} bind:this={deleteButton} onclick={confirmDelete}>Delete event…</button>{/if}
    </section>{/if}
  {/if}
</main>
